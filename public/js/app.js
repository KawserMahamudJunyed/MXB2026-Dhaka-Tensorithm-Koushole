...
document.getElementById('book-upload-input').addEventListener('change', async function (e) {
    if (e.target.files && e.target.files[0]) {
        ...
        try {
            ...
            // 3. Insert metadata into DB with timeout and retry logic
            ...
            const withTimeout = (promise, ms) => Promise.race([
                promise,
                new Promise((_, reject) => setTimeout(() => reject(new Error('Database timeout - Supabase not responding')), ms))
            ]);
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
                ...
                try {
                    ...
                    console.error(`Database Insert Timeout (Attempt ${attempt}):`, timeoutErr);
            ...
            if (insertError) {
                ...
                alert(`Failed to save book to library: ${errorMsg}\n\nFile was uploaded to storage. Please try again or check Supabase connection.`);
                throw insertError;
            }
...
function getColorFromGradient(gradientClass) {
    ...
    if (gradientClass.includes('rose')) return '#F43F5E';
    ...
    if (gradientClass.includes('sky')) return '#0EA5E9';
...
async function sendMessage() {
    ...
    if (isRagMode) {
        ...
        bookContextLabel = `<div class="text-[10px] text-indigo-900 font-bold mt-1 border-t border-black/10 pt-1"><i class="fas fa-book"></i> ${selectedOption.text}</div>`;
    }
